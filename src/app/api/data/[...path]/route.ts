...
async function getUserId(request: NextRequest): Promise<string | null> {
    ...
    if (authHeader?.startsWith('Bearer ')) {
        ...
        try {
            ...
            // The check for revocation 'true' is important for security.
            const decodedToken = await getAdminApp().auth().verifyIdToken(idToken, true); 
            return decodedToken.uid;
        } catch (error) {
            console.error("Error verifying ID token:", error);
            // The error might be because it's a custom token or an invalid one.
            // In either case, for this data route, the user is not authorized.
            return null;
        }
    }
    return null;
