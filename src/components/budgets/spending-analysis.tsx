...
export function SpendingAnalysis({
  ...
  const analysisData = useMemo(() => {
    ...
    const recentTransactions = allTransactions.filter((t: Transaction) => {
      ...
      return (
        isAfter(transactionDate, sixMonthsAgo) &&
    ...
    const categoryGroups = recentTransactions.reduce(
    ...
    const categoryAnalysis: CategoryAnalysis[] = Object.entries(
      categoryGroups
    ).map(([category, categoryTransactions]: [string, Transaction[]]) => {
      ...
      const recentSpending =
        recentPeriod.reduce(
          (sum: number, t: Transaction) => sum + t.amount,
          0
        ) / 3; // média mensal recente
      const olderSpending =
        olderPeriod.reduce((sum: number, t: Transaction) => sum + t.amount, 0) /
        3; // média mensal anterior
      ...
      if (trend === "up") {
        ...
        suggestedBudget = averageMonthly * 1.1;
      }
      ...
      <Card>
        <CardContent sx={{ p: 4, textAlign: "center" }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Adicione algumas transações para receber sugestões de orçamento
            baseadas no seu histórico de gastos.
          </Typography>
        </CardContent>
      </Card>
    );
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case "up":
        return (
          <TrendingUp
            style={{ width: 16, height: 16, color: theme.palette.error.main }}
          />
        );
      case "down":
        return (
          <TrendingDown
            style={{ width: 16, height: 16, color: theme.palette.success.main }}
          />
        );
      default:
        return (
        ...
        <CardContent>
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 6, md: 3 }}>
              ...
              >
                <Typography variant="h5" fontWeight="bold">
                  {formatCurrency(analysisData.averageMonthlyTotal)}
                </Typography>
                <Typography variant="caption" color="text.secondary">
                  Média Mensal
                </Typography>
          ...
          <Stack spacing={3}>
            {analysisData.categories.map((category, index) => {
              const percentageOfTotal =
                (category.averageMonthly / analysisData.averageMonthlyTotal) *
                100;
                      ...
                      <Stack direction="row" alignItems="center" spacing={1.5}>
                        ...
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            ...
                            {percentageOfTotal.toFixed(1)}% dos gastos
                          </Typography>
                        </Box>
                      </Stack>
                      <Stack direction="row" alignItems="center" spacing={1}>
                    ...
                    <Grid container spacing={2} sx={{ textAlign: "center" }}>
                      ...
                      <Grid size={{ xs: 12, md: 4 }}>
                        <Typography
                          ...
                          color="primary.main"
                        >
                          {formatCurrency(category.suggestedBudget)}
                        </Typography>
                        <Typography variant="caption" color="text.secondary">
                          Orçamento Sugerido
                        </Typography>
                      ...
                      >
                        ...
                        <Box>
                          <Typography variant="body2" color="error.dark">
                            <Box component="strong">Atenção:</Box> Gastos
                            crescentes nesta categoria.
                          </Typography>
                          <Typography variant="caption" color="error.dark">
                            Monitore de perto para evitar que o orçamento seja
                            estourado.
                          </Typography>
                        </Box>
                      </Paper>
            ...
            >
              <Box>
                <Typography variant="subtitle1" fontWeight="medium">
                  Total dos Orçamentos Sugeridos
                </Typography>
                <Typography variant="caption" color="text.secondary">
              ...
              <Box sx={{ textAlign: "right" }}>
                <Typography variant="h5" fontWeight="bold">
                  {formatCurrency(
                    analysisData.categories.reduce(
                ...
                <Typography variant="caption" color="text.secondary">
                  vs {formatCurrency(analysisData.averageMonthlyTotal)} gasto
                  atual
                </Typography>
              </Box>
            </Paper>
