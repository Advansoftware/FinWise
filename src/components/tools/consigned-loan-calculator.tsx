...
export function ConsignedLoanCalculator({ payrollData }: ConsignedLoanProps) {
  ...
  const [employeeType, setEmployeeType] = useState<"clt" | "public" | "inss">(
    "clt"
  );
  ...
  const getMarginRate = (type: string) => {
    switch (type) {
      case "clt":
        return 0.3; // 30% para CLT
      case "public":
        return 0.35; // 35% para servidor público
      case "inss":
        return 0.45; // 45% para aposentado/pensionista
      default:
        return 0.3;
    }
  };
  ...
  const calculateLoan = () => {
    ...
    const maxLoanAmount =
      (availableMargin * (Math.pow(1 + monthlyRate, termMonths) - 1)) /
  ...
  const getEmployeeTypeLabel = (type: string) => {
    switch (type) {
      case "clt":
        return "CLT (30% margem)";
      case "public":
        return "Servidor Público (35% margem)";
      case "retired":
        return "Aposentado/Pensionista (45% margem)";
      default:
        return type;
    }
  };
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 4 }}>
                <Box>
                  <Typography variant="caption" color="info.dark">
                    Margem Total (
                    {formatPercentage(getMarginRate(employeeType) * 100)}):
                  </Typography>
                  <Typography variant="body2" fontWeight="medium">
                    {formatCurrency(
                      payrollData.netSalary * getMarginRate(employeeType)
                    )}
                  </Typography>
                </Box>
              </Grid>
            ...
            <Grid size={{ xs: 12, sm: 6 }}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setEmployeeType(e.target.value as "clt" | "public" | "inss")
                  }
                >
                  <MenuItem value="clt">CLT - 30% margem</MenuItem>
                  <MenuItem value="public">
                    Servidor Público - 35% margem
                  </MenuItem>
                  <MenuItem value="inss">
                    Aposentado/Pensionista - 45% margem
                  </MenuItem>
                </Select>
              </FormControl>
            </Grid>
            ...
            <Stack spacing={3}>
              ...
              <Box>
                <Typography variant="h6" gutterBottom>
                  Simulação - {getEmployeeTypeLabel(employeeType)}
                </Typography>
              ...
              {calculation.monthlyPayment > calculation.availableMargin && (
                ...
                >
                  ...
                  <Typography variant="body2" color="error.dark">
                    Valor da parcela excede a margem disponível. Reduza o valor
                    ou aumente o prazo.
                  </Typography>
                </Paper>
              )}
              ...
              >
                ...
                <Stack spacing={0.5}>
                  <Typography variant="caption" color="warning.dark">
                    • Desconto direto na folha de pagamento garante menores
                    taxas
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Margem consignável: CLT (30%), Servidor (35%), Aposentado
                    (45%)
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Taxas variam entre 1,5% a 3,5% a.m. dependendo do banco
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Simulação considera Sistema Price (parcelas fixas)
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Consulte seu banco para condições específicas
                  </Typography>
                </Stack>
              </Paper>
            </Stack>
