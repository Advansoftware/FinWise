...
export function IncomeTaxCalculator({ payrollData }: IncomeTaxCalculatorProps) {
  ...
  const [calculation, setCalculation] = useState<IncomeTaxCalculation | null>(
    null
  );
  ...
  const calculateIncomeTax = () => {
    ...
    const monthlyINSS =
      ...
      registeredINSS ||
      (() => {
        if (grossMonthly <= 1412.0) return grossMonthly * 0.075;
    ...
    const maxEducationDeduction = 3561.5 * 12; // Limite anual educação
    const educationDeductionAnnual = Math.min(
      educationExpenses,
      maxEducationDeduction
    );
    ...
    const effectiveRate =
      grossMonthly > 0 ? (monthlyIR / grossMonthly) * 100 : 0;
          ...
          {calculation && (
            <Stack spacing={3}>
              ...
              {(() => {
                const registeredIR = getIRFromPayroll(payrollData);
                const registeredINSS = getINSSFromPayroll(payrollData);
                const calculatedIR = calculateIRFromSalary(
                      ...
                      >
                        ...
                        <Stack spacing={0.5}>
                          <Typography variant="body2">
                            IR do seu holerite:{" "}
                            <strong>{formatCurrency(registeredIR)}</strong>
                          </Typography>
                          <Typography variant="body2">
                            IR calculado pela tabela:{" "}
                            <strong>{formatCurrency(calculatedIR)}</strong>
                          </Typography>
                          {Math.abs(registeredIR - calculatedIR) <= 20 ? (
                            ...
                            <Typography variant="caption" color="warning.main">
                              ⚠️ Diferença pode indicar dependentes ou outras
                              deduções não consideradas
                            </Typography>
                          )}
                        </Stack>
                      </Paper>
                    ...
                    {registeredINSS > 0 && (
                      ...
                      >
                        <Typography variant="body2" color="info.main">
                          💡 <strong>Usando dados do seu holerite:</strong> INSS
                          de {formatCurrency(registeredINSS)}
                          {registeredIR > 0 &&
                            ` e IR de ${formatCurrency(registeredIR)}`}
                        </Typography>
                      </Paper>
                    )}
                  </Stack>
              ...
              <Paper
                sx={{ p: 2, bgcolor: alpha(theme.palette.warning.main, 0.1) }}
              >
                <Typography
                  variant="subtitle2"
                  color="warning.dark"
                ...
                <Stack spacing={0.5}>
                  <Typography variant="caption" color="warning.dark">
                    • Cálculo baseado na tabela 2024/2025 da Receita Federal
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Deduções: INSS, dependentes (R$ 189,59), saúde
                    (ilimitado), educação (até R$ 3.561,50)
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Estimativa de restituição considera apenas IR na fonte vs
                    IR devido
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
                    • Para declaração completa, considere outras rendas e
                    deduções
                  </Typography>
                  <Typography variant="caption" color="warning.dark">
