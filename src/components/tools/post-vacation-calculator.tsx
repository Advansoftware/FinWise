...
import {Card, CardContent, CardHeader, Typography, TextField, Button, Divider, Box, Stack, Paper, Chip, useTheme, alpha, InputAdornment} from '@mui/material';
import { CalendarDays, Calculator, Info, AlertTriangle, CheckCircle } from "lucide-react";
      ...
      <CardHeader
        title={
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <CalendarDays style={{ width: '1.25rem', height: '1.25rem', color: theme.palette.primary.main }} />
      ...
      <CardContent>
        ...
        {result && (
          <Stack spacing={3} sx={{ pt: 2, borderTop: 1, borderColor: 'divider' }}>
            <Typography variant="subtitle2">Simulação do Holerite Pós-Férias:</Typography>
            
            <Stack spacing={3}>
              {/* Valores base */}
              <Paper variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                <Typography variant="caption" color="text.secondary" fontWeight="bold" sx={{ mb: 2, display: 'block' }}>📋 Composição do Holerite:</Typography>
                
                <Stack spacing={1}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="caption" color="text.secondary">Salário normal (30 dias):</Typography>
                    <Chip label={formatCurrency(result.normalSalary)} size="small" variant="outlined" />
                  </Box>
                  
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="caption" color="text.secondary">Desconto pelos {vacationDays} dias não trabalhados:</Typography>
                    <Chip label={`-${formatCurrency(result.vacationDiscount)}`} size="small" variant="outlined" color="error" />
                  </Box>
                  
                  <Divider sx={{ my: 1 }} />
                  
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="caption" fontWeight="medium">Valor pelos {result.daysWorkedAfterVacation} dias trabalhados:</Typography>
                    <Chip label={formatCurrency(result.grossPayroll)} size="small" variant="outlined" sx={{ fontWeight: 'bold' }} />
                  </Box>
                </Stack>
              </Paper>
              ...
              <Paper variant="outlined" sx={{ p: 2, bgcolor: alpha(theme.palette.error.main, 0.1), borderColor: alpha(theme.palette.error.main, 0.2) }}>
                <Typography variant="caption" color="error.main" fontWeight="bold" sx={{ mb: 2, display: 'block' }}>💸 Descontos Aplicados (Proporcionais):</Typography>
                
                <Stack spacing={1}>
                  {result.detailedDiscounts.inss > 0 && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="caption" color="text.secondary">INSS:</Typography>
                  ...
                  {result.detailedDiscounts.ir > 0 && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="caption" color="text.secondary">Imposto de Renda:</Typography>
                  ...
                  {result.detailedDiscounts.otherDiscounts.map((discount, index) => (
                    <Box key={index} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <Chip label={`-${formatCurrency(discount.amount)}`} size="small" variant="outlined" color="error" />
                    </Box>
                  ))}
                  ...
                  {result.consignedDiscount > 0 && (
                    <>
                      ...
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Chip label={`-${formatCurrency(result.consignedDiscount)}`} size="small" variant="outlined" color="error" />
                      </Box>
                    </>
                  )}
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Typography variant="caption" fontWeight="medium">Total dos descontos:</Typography>
                    <Chip label={`-${formatCurrency(result.totalDiscounts)}`} size="small" variant="outlined" color="error" sx={{ fontWeight: 'bold' }} />
                  </Box>
                </Stack>
              </Paper>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', pt: 1, borderTop: 1, borderColor: 'divider' }}>
                <Typography variant="subtitle2">Salário Líquido Pós-Férias:</Typography>
                <Chip 
              ...
              <Paper variant="outlined" sx={{ p: 2, bgcolor: alpha(theme.palette.info.main, 0.1), borderColor: alpha(theme.palette.info.main, 0.2) }}>
                ...
                <Stack spacing={0.5}>
                  <Typography variant="caption" color="text.secondary">• Você recebe o salário normal de 30 dias: {formatCurrency(result.normalSalary)}</Typography>
                  <Typography variant="caption" color="text.secondary">• É descontado apenas os {vacationDays} dias que NÃO trabalhou: -{formatCurrency(result.vacationDiscount)}</Typography>
                  <Typography variant="caption" color="text.secondary">• Sobram os {result.daysWorkedAfterVacation} dias que você trabalhou: {formatCurrency(result.grossPayroll)}</Typography>
                  <Typography variant="caption" color="text.secondary">• Descontos são aplicados proporcionalmente aos dias trabalhados</Typography>
                  <Typography variant="caption" color="text.secondary">• Empréstimo consignado continua o valor fixo do holerite</Typography>
                  {result.netPayroll >= 0 ? (
                    <Typography variant="caption" color="success.main" fontWeight="medium" sx={{ mt: 1, display: 'block' }}>
                      ✅ Resultado positivo: você receberá {formatCurrency(result.netPayroll)}
                    </Typography>
                  ) : (
                    <Typography variant="caption" color="error.main" fontWeight="medium" sx={{ mt: 1, display: 'block' }}>
